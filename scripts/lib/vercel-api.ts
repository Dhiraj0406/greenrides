...
const TEAM_ID    = "team_2kOfHI6diUznVNSYCoXLduWL";
...
export async function rollbackToPreviousDeployment(): Promise<string> {
  ...
  const res = await vercelFetch(`/v9/projects/${PROJECT_ID}/rollback/${previous.uid}?${teamQs()}`, { method: "POST" });
  if (!res.ok) {
