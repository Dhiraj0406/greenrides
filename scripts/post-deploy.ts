...
async function updateLog(db: ReturnType<typeof getSupabase> | null, logId: string, data: Record<string, unknown>) {
  if (!db || logId === "none") return;
  await db.from("ImprovementLog").update(data).eq("id", logId);
}

async function notify(text: string) {
  if (!process.env.TELEGRAM_BOT_TOKEN) {
    console.log(`[Telegram skipped] ${text.replace(/<[^>]+>/g, "")}`);
    return;
  }
...
async function main() {
  ...
  const hasSupabase = !!(process.env.NEXT_PUBLIC_SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY) && logId !== "none";
  const db = hasSupabase ? getSupabase() : null;
  if (!hasSupabase) console.log("ℹ️  Supabase not available — skipping log updates");
  ...
  if (deployment.readyState === "ERROR") {
    ...
    await updateLog(db, logId, { status: "failed", deployment_id: deployment.uid, notes: "Vercel build error" });
    await notify(`⚠️ <b>Day ${day} — build failed</b>\n\n${title}\n\nVercel build error. Check Vercel dashboard. Previous version untouched.`);
    process.exit(1);
  }
  ...
  updateBacklogItem(day, { status: "completed", deployment_id: deployment.uid });
