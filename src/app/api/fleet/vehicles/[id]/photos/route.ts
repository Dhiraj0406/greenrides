...
export async function POST(
  ...
  if (!ALLOWED.has(file.type))  return Response.json({ error: "Images only (JPEG, PNG, WebP)" }, { status: 400 });
  ...
  if (updateErr) {
    ...
    return Response.json({ error: "Failed to save photo reference" }, { status: 500 });
  }
...
export async function DELETE(
  ...
  if (!path || !currentPhotos.includes(path)) {
    return Response.json({ error: "Photo path not found" }, { status: 400 });
  }
  ...
  if (removeErr) {
    ...
    return Response.json({ error: "Failed to remove photo from storage" }, { status: 500 });
  }
  ...
  if (updateErr) {
    return Response.json({ error: "Failed to remove photo reference" }, { status: 500 });
  }
